...
CMBase.DATA_TYPE_BOOLEAN_ARRAY=12; // 
...
CMBase.prototype.SetSettings=function(NewSettings)
{
	...
	for (var GroupKey in NewSettings)
	{
		var TheGroup=NewSettings[GroupKey];
	
		if (this.Settings[GroupKey]==undefined) this.Settings[GroupKey]={};
		
		for (var SettingKey in TheGroup)
		{
			this.Settings[GroupKey][SettingKey]=TheGroup[SettingKey];
		}
	}
...
CMBase.prototype.GetSetting=function(Group,Key,Default)
{
	...
	{
		if (this.Settings[Group]!=undefined) 
		{
			if (this.Settings[Group][Key]!=undefined)
			{
				Result=this.Settings[Group][Key];
			}
		}
	}
	return(Result);
...
CMBase.prototype.SetSetting=function(Group,Key,Value)
{
	var TheSettings={};
	TheSettings[Group]={};
	TheSettings[Group][Key]=Value;
	
	this.SetSettings(TheSettings);
	
//	if (this.Settings==undefined) this.Settings={};
	
//	if (this.Settings[Group]==undefined) this.Settings[Group]={};
...
CMDatasetVector.prototype.GetIcon=function(TheLayer,Default)
{
	...
	if ((this.TheRegions!=null)) //
	{
		...
		case CMDatasetVector.TYPE_POINTS:
			{
				var TheType=this.GetSetting("Mark","Type",undefined);
...
CMDatasetVector.prototype.AddPolyline=function(Xs,Ys,Zs)
{
	...
	{
		this.TheRegions=[];
		this.Type=CMDatasetVector.TYPE_POLYLINES;
	}
	var ThePoly=[Xs,Ys];
...
function CMDatasetRaster() 
{
	...
	this.TheMatrix=null; // used to determine when data is available
}
...
CMDatasetRaster.prototype.GetPathArray=function(Xs,Ys,Zs,ZOffset,ZMultiplier) 
{
	...
	else
	{
		...
		var PixelWidth=this.WidthInPixels/TheData.length;
		var PixelHeight=this.HeightInPixels/TheData[0].length;
		...
		for (var j=0;j<Xs.length;j++)
		{
			...
			if ((j!=0)&&(false)) // have another coordinate
			{
				var RowInc=1;
				var ColInc=1;
				...
				{
					if (StartRefY<EndRefY) // going from left up to the right
					{
					}
					else // going from left down to the right
					{
						RowInc=-1;
					}
				}
				else // going right to left
				{
					...
					if (StartRefY<EndRefY) // going from right up to the left
					{
					}
					else // going from right down to the left
					{
						RowInc=-1;
					}
				...
				var StartRow=Math.ceil((StartRefY-this.TheBounds.YMin)/PixelHeight);
				var EndRow=Math.floor((EndRefY-this.TheBounds.YMin)/PixelHeight);
				
				if (RowInc<0) // y decreasing with x increasing
				{
					StartRow=Math.floor((StartRefY-this.TheBounds.YMin)/PixelHeight);
					EndRow=Math.ceil((EndRefY-this.TheBounds.YMin)/PixelHeight);
				}
				
				var StartRowY=(StartRow*PixelHeight)+this.TheBounds.YMin;
				var EndRowY=(EndRow*PixelHeight)+this.TheBounds.YMin;
				
				// skip the first and last row if they are the same as the start and/or last
				if (StartRowY==StartRefY) StartRow+=RowInc;
				if (EndRowY==EndRefY) EndRow-=RowInc;
				...
				{
					// find the row number in the raster
					var Row=StartRow+(RowInc*RowCount);
					
					// find the ref of the top of the row
					var RowRefTopY=this.TheBounds.YMin+(Row*PixelHeight);
					
					// find the intersection with the top of this row
					
					var RowRefTopIntersectionX=(RowRefTopY-b)/m;
					
					EndRefZ=this.GetSampleFromRef(RowRefTopIntersectionX,RowRefTopY)*ZMultiplier+ZOffset;
					PathArray.push(new THREE.Vector3(RowRefTopIntersectionX,RowRefTopY,EndRefZ));
					
					var StartCol=Math.floor((StartRefX-this.TheBounds.XMin)/PixelWidth);
					var ColRemainder=RefX-(StartCol*PixelWidth);
				}
			}
			// setup the coordinate's z value
			var EndRefZ=0;
			if (Zs!=null) EndRefZ=Zs[j];
			else 
			{
				EndRefZ=this.GetSampleFromRef(EndRefX,EndRefY);
				if (EndRefZ==undefined) EndRefZ=0;
			}
			EndRefZ=EndRefZ*ZMultiplier+ZOffset;
			PathArray.push(new THREE.Vector3(EndRefX,EndRefY,EndRefZ));
...
function CMDatasetPyramid() 
{
	...
	this.AttributeInfo=null; // implemented differently here than in the GeoJSON code
...
*
*/
CMDatasetPyramid.prototype.GetDataForTextureTile=function(TheView,TheTextureTile,Exaggeration)
{
	var Result=null;
	...
	for (var Row=0;(Row<this.NumRows)&&(Result==null);Row++)
	{
		for (Column=0;(Column<this.NumColumns)&&(Result==null);Column++)
		{
			...
			Result=TheTile.GetDataForTextureTile(TheView,TheTextureTile,Exaggeration);
		}
	}
	return(Result);
}
...
CMDatasetPyramid.prototype.Paint=function(TheLayer,TheView,SelectedOnly,MouseOverOnly) 
{
	...
	{
//		TheView.GetParent(CMMainContainer).AddToDebugPanel(this.GetName()+" Starting Paint:"+CMUtilities.GetSeconds());
...
//		TheView.GetParent(CMMainContainer).AddToDebugPanel(this.GetName()+" Leaving Paint:"+CMUtilities.GetSeconds());
	}
}
//******************************************************************
// Searching
...
CMDatasetPyramid.prototype.GetSearchResults=function(SearchPhrase,ResultsPanel) 
{
	...
	if ((this.IsVisible())&&(this.AttributeInfo!=null))
	{
		var NumRows=this.GetNumAttributeRows();
		...
		for (var ColumnIndex=0;ColumnIndex < NumColumns; ColumnIndex++) 
		{
			var TheColumn=this.Attributes.Values[ColumnIndex]; // needs request
...
function CMDatasetPyramidOpenFormat() 
{
	...
	this.Visible=true; // keeps track of visibility so we know when to make the tiles invisible
...
CMDatasetPyramidOpenFormat.prototype.GetFileExtension=function() 
{
...
{ 
	if ((this.Visible)&&(Flag==false))
	{
		this.TopTile.HideTiles(TheLayer);
	}
	this.Visible=Flag;
}
...
CMTileOpenFormat.prototype.GetTileImageFilePath=function() 
{
	...
	var FileExtension=this.TheDataset.GetFileExtension();
...
* @private
*/
CMTileOpenFormat.prototype.LoadTileRaster=function(TheView) 
{
...
CMTileOpenFormat.prototype.CheckPaintTile=function(TheView,ThisStepTileRefWidth,TheLayer) 
{
	...
	else // otherwise, check if this tile needs to be painted
	{
		if (PaintTile==false) // see if we should 
		{
			...
			{
				var Distance=CMTileOpenFormat.GetDistanceToView(TheView,this.GetTileExtent(),TheLayer);
				
				if (Distance>ThisStepTileRefWidth*2) PaintTile=true;
						...
						for (var RowIndex=0;RowIndex<2;RowIndex++)
						{
							for (var ColumnIndex=0;ColumnIndex<2;ColumnIndex++)
							{
								...
								if (PaintChildren) // have an existing child tile
								{
									...
									{
										TheChildTile.LoadTileRaster(TheView);
									}
									else 
									{
										...
										else 
										{
											...
											{
												this.LoadTileRaster(TheView); // try again
											}
										}
									}
								}
...
/**
* Utility function to get the extent of this tile.
* @private
*/
CMTileOpenFormat.prototype.GetTileExtent=function() 
{
...
* @private
*/
CMTileOpenFormat.GetDistanceToView=function(TheView,Extent,TheLayer) 
{
	var CameraPosition=TheView.GetPosition();
	
	// find the center of the tile
	var	OX=(Extent.XMin+Extent.XMax)/2;
	var OY=(Extent.YMin+Extent.YMax)/2;
	var OZ=0;
	
	if (true)
	{
		var Vector1=CMTileOpenFormat.GetGlobalCoordinate(TheLayer,OX,OY,0);
...
CMTileOpenFormat.prototype.SetupGeometry=function(TheLayer) 
{
	...
	for (var Row=0;Row<=16;Row++)
	{
		...
		for (var Column=0;Column<=16;Column++)
		{
			...
			var Northing=YMin+(Row*RowRefWidth);
			var Easting=XMin+(Column*ColumnRefWidth);
			
			var Result=TheGoogleMapsProjector.ProjectToGeographic(Easting,Northing);
...
* @priate
*/
CMTileOpenFormat.prototype.HideMesh=function(TheLayer) 
{
	if (this.MeshIsInGeo) // need to remove this tile from mesh
	{
		var TheGeo=TheLayer.GetParent(CMGeo);
		TheGeo.RemoveOGLObjectFromGeo(this.TheMesh);
		this.MeshIsInGeo=false;
	}
}
//******************************************************************
...
//******************************************************************
/**
* Hides this tile and all child tiles
* @protected
*/
CMTileOpenFormat.prototype.HideTiles=function(TheLayer) 
{ 
			...
			if (this.ChildTiles[RowIndex]!=null)
			{
				for (var ColumnIndex=0;ColumnIndex<2;ColumnIndex++)
				{
					if (this.ChildTiles[RowIndex][ColumnIndex]!=null)
					{
						...
						TheChildTile.HideTiles(TheLayer);
					}
				}
			}
		}
...
CMTileOpenFormat.prototype.PaintTile=function(TheLayer,TheView,SelectedOnly) 
{
	...
	if ((typeof(CM3View)!="undefined")&&(TheView instanceof CM3View)) // have a 3d view
	{
		if ((TheLayer.GetVisible()))
		{
			...
			var CenterY=(Extent.YMax+Extent.YMax)/2;
			var CenterX=(Extent.XMax+Extent.XMax)/2;
			
			var Vector1=CMTileOpenFormat.GetGlobalCoordinate(TheLayer,CenterX,Extent.YMin,0);
			var Vector2=CMTileOpenFormat.GetGlobalCoordinate(TheLayer,CenterX,Extent.YMax,0);
...
// 				if ("RasterExtension" in this.TheData)  // raster file
				{
					if (this.MeshIsInGeo==false) // the mesh is not in the Geo, we need to add it
					{
						if (this.TheMesh==null) // the mesh is not available, we need to create it
						{
							...
							var TextureDataset=this.TheDataset;
	//						var DEMDataset=TheLayer.GetDEMDataset();
							var Exaggeration=TheLayer.GetSetting("Elevation","Exaggeration",1);
							
//							var TheMatrixes=DEMDataset.GetDataForTextureTile(TheView,this,Exaggeration);
...
//								TheGeometry.SetPositions(TheArray);
							...
							texloader.crossOrigin = '';
								...
								//SolidColorMaterial.map=new THREE.Texture(TheImage); // image
...
//							TheGeo.AddOGLObjectToGeo(helper);
	...
	//**************************************************************
	// 2D painting
	else if(CMUtilities.BoundsOverlap(Extent,TheView.GetExtent()))
	{
...
CMItem.SettingDefintions=
{
	Item:
	{
		...
		Visible: { Name:"Visible",Type:CMBase.DATA_TYPE_BOOLEAN, Default:true }, // True to make the item visible
		Selectable: { Name:"Selectable",Type:CMBase.DATA_TYPE_BOOLEAN, Default:true }, // True to allow the user to select the item
	...
	Style:
	{
		...
		GradientRadius: { Name:"Gradient Radius",Type:CMBase.DATA_TYPE_FLOAT, Default:100 }, // Pixel radius for radia gradient (HTML Canvas)
	...
	Text:
	{
		...
		Visible: { Name:"Text Visible",Type:CMBase.DATA_TYPE_BOOLEAN, Default:true },
...
CMItem.prototype.SetSettings=function(NewSettings,TimeSlice) 
{
	...
	//	this.TimeSlices[TimeSliceIndex].Settings=NewSettings; // save them into the object
		...
		for (var GroupKey in NewSettings)
		{
			...
			for (var SettingKey in TheGroup)
			{
				TheSettings[GroupKey][SettingKey]=TheGroup[SettingKey];
			}
		}
	}
	this.SettingsChanged();
...
{
	var Current=this.GetSetting("Item","Visible");
	
	if (Current!=Flag) // works for true, false, and undefined
	{
		this.SetSetting("Item","Visible",Flag);
		this.Repaint();
	}
}
/**
...
{
	var Result=this.GetSetting("Item","Visible",true);
...
CMItemPoly.prototype.MouseDown=function(TheView,RefX,RefY,TheEvent) 
{
	...
	if ((Used==false)&&(this.Creating)) // either creating
	{
		...
		if (TheEvent.detail==2) // double click, remove the last point and 
		{
			...
			ControlPoints.Smoothing.pop(); // remove the last point which was added by the second button click
		}
		else // add a point
		{
			...
			else // add the point
			{
				...
				ControlPoints.Smoothing.splice(NumPoints-1,0,ControlPoints.Smoothing[NumPoints-1]); // duplicate the last points smoothing setting
			}
		}
	...
	else if (Used==false)
	{
		...
		else // right buttonclicked
		{
			...
			if (SelectedPart!=-1)
			{
				...
				{
					this.TheItem.DeleteControlPoint(this.SelectedPart);
				...
				DeleteElement.addEventListener('click', function(event)
				{
					this.TheItem.DuplicateControlPoint(this.SelectedPart);
...
CMItemPoly.prototype.Paint=function(TheView) 
{
	...
	else
	{
		...
		if (this.Closed==false) NumCurves=Xs.length-1;
...
//		Y1s.push(Ys[1][0]);
		...
		for (var Curve=0;Curve<NumCurves;Curve++)
		{
			...
			if ((Curve!=0)||(this.Closed)) StartSmooth=Smoothing[StartPoint]; // allow smoothing once past first point or if closed
			
			var EndSmooth=false;
			if ((Curve+1<NumPoints-1)||(this.Closed)) EndSmooth=Smoothing[EndPoint]; // not at the end
...
//					TheView.PaintRefLine(Points[0][StartPoint],Points[1][StartPoint],Xs[EndPoint],Ys[EndPoint]);
				}
			...
			else // start smooth, draw a 3rd order or 4th order Bezier 
			{
				if (EndSmooth) // both smooth, draw a 4th order Bezier
				{
					...
					X1s.push(Xs[EndPoint]);
					Y1s.push(Ys[EndPoint]);
//					this.DrawCurve(TheView,Xs,Ys,Points,StartPoint,EndPoint,X1s,Y1s);
				}
				else // start smooth, end sharp
				{
					...
					X1s.push(Xs[EndPoint]);
					Y1s.push(Ys[EndPoint]);
///					this.DrawCurve(TheView,Xs,Ys,Points,EndPoint,StartPoint,X1s,Y1s);
				}
			}
		}
		
/*		for (var i=0;i+1<X1s.length;i++)
		{
			TheView.PaintRefLine(X1s[i],Y1s[i],X1s[i+1],Y1s[i+1]);
		}
*/		
//		if (this.Closed)
		{
...
/*		
		var Points=CMUtilityBezier.GetThreePoint3D(10,Xs[0],Ys[0],0,Xs[1],Ys[1],0,Xs[2],Ys[2],0);
		
		this.DrawCurve(TheView,Xs,Ys,Points,0,1);
		
		// draw the inbetween Bezier curves
		
		for (var ControlIndex=1;ControlIndex<Xs.length-2;ControlIndex++)
		{
			var Points=CMUtilityBezier.GetFourPoint2D(10,Xs[ControlIndex-1],Ys[ControlIndex-1],
					Xs[ControlIndex],Ys[ControlIndex],Xs[ControlIndex+1],Ys[ControlIndex+1],
					Xs[ControlIndex+2],Ys[ControlIndex+2]);
			
			this.DrawCurve(TheView,Xs,Ys,Points,ControlIndex,ControlIndex+1);
		}
		...
		var Points=CMUtilityBezier.GetThreePoint3D(10,Xs[LastPoint],Ys[LastPoint],0,
					Xs[LastPoint-1],Ys[LastPoint-1],0,Xs[LastPoint-2],Ys[LastPoint-2],0);
		
		this.DrawCurve(TheView,Xs,Ys,Points,LastPoint,LastPoint-1);
*/	}
...
CMItemPoly.prototype.GetControlPoints=function(TimeSlice)
{
	...
	{
		if (Settings.Curve.Coordinates!=undefined)
		{
			var Xs=Settings.Curve.Coordinates.Xs;
			var Ys=Settings.Curve.Coordinates.Ys;
			var Smoothing=Settings.Curve.Smoothing;
			
			Result={Xs:Xs,Ys:Ys,Smoothing:Smoothing};
		}
	}
...
CMItemPoly.prototype.SetControlPoints=function(TimeSlice,Xs,Ys,Smoothing)
{
	...
	if (Settings!=undefined)
	{
		if (Settings.Curve.Coordinates==undefined) Settings.Curve["Coordinates"]={};
		if (Settings.Curve.Smoothing==undefined) Settings.Curve["Smoothing"]=[];
		
		Settings.Curve.Coordinates.Xs=Xs;
		Settings.Curve.Coordinates.Ys=Ys;
		Settings.Curve.Smoothing=Smoothing;
	}
	else
	{
...
CMItemPoly.prototype.DeleteControlPoint=function(SelectedPart)
{
	var TheScene=this.GetParent(CMScene);
	...
	ControlPoints.Xs.splice(SelectedPart,1); // delete the point clicked on
	ControlPoints.Ys.splice(SelectedPart,1);
	ControlPoints.Smoothing.splice(SelectedPart,1);
				
	this.SetControlPoints(MinTime,ControlPoints.Xs,ControlPoints.Ys,ControlPoints.Smoothing);

}

CMItemPoly.prototype.DuplicateControlPoint=function(SelectedPart)
{
	var TheScene=this.GetParent(CMScene);
	...
	ControlPoints.Xs.splice(SelectedPart,0,ControlPoints.Xs[SelectedPart]); // duplicate the point clicked on
	ControlPoints.Ys.splice(SelectedPart,0,ControlPoints.Ys[SelectedPart]);
	ControlPoints.Smoothing.splice(SelectedPart,0,ControlPoints.Smoothing[SelectedPart]);
...
CMItemPoly.prototype.InPart=function(TheView,RefX,RefY,RefTolerance)
{
	...
	if (Result!=null)
	{
		...
		// find the first part that is clicked in
		for (var i=0;(i<X1s.length)&&(ThePart==-1);i++)
		{
			var X=X1s[i];
			var Y=Y1s[i];
...
CMItemPolyArrow.SettingDefintions=
{
	Arrow:
	{
		BarbWidth: { Name:"Barb Width",Type:CMBase.DATA_TYPE_INTEGER, Default:2 },
		BarbLength: { Name:"Barb Length",Type:CMBase.DATA_TYPE_INTEGER, Default:3 },
		ShaftWidth: { Name:"Shaft Width",Type:CMBase.DATA_TYPE_INTEGER, Default:2 },
		HeadLength: { Name:"Head Length",Type:CMBase.DATA_TYPE_INTEGER, Default:4 },
	},
};
...
function CMItemPolyArrow() 
{
	...
	// Dimensions of the arrow head (cached for speed)
	this.BarbWidth=5;
	this.BarbLength=5;
	this.ShaftWidth=2;
	this.HeadLength=4;
...
CMItemPolyArrow.prototype.FindCoordinates=function() 
{
	...
	//		DrawBezier(TheView,Xs[LastControlIndex],Ys[LastControlIndex],Points3,Xs[LastControlIndex-1],Ys[LastControlIndex-1]);
			...
			for (var i=0;(i<Points3[0].length)&&(Length<this.ShaftWidth);i++)
			{
...
//		var LineDY=Ys[LastControlIndex]-Points3[1][0];
		...
		var Result=GetPointsOffLine(Xs[LastControlIndex],Ys[LastControlIndex],Angle,this.BarbWidth,this.BarbLength);
		...
		var Result=GetPointsOffLine(Xs[LastControlIndex],Ys[LastControlIndex],Angle,this.ShaftWidth,this.HeadLength);
		...
		var BannerPoints=FindBannerPoints(X1s[0],Y1s[0],X1s[0],Y1s[0],X1s[1],Y1s[1],this.ShaftWidth);
//		var BannerPoints=FindBannerPoints(Xs[0],Ys[0],Xs[0],Ys[0],Points1[0][1],Points1[1][1],this.ShaftWidth);
		...
		{
			var BannerPoints=FindBannerPoints(X1s[i],Y1s[i],X1s[i],Y1s[i],X1s[i+1],Y1s[i+1],this.ShaftWidth);
...
CMItemPolyArrow.prototype.Private_SetupProperties=function(TheView,TimeSlice) 
{
	...
	this.BarbWidth=Settings1.Arrow.BarbWidth;
	this.BarbLength=Settings1.Arrow.BarbLength;
	this.ShaftWidth=Settings1.Arrow.ShaftWidth;
	this.HeadLength=Settings1.Arrow.HeadLength;
	...
	if (Result[1]!=null)
	{
		...
		this.BarbWidth=((Settings1.Arrow.BarbWidth*(1-Factor))+(Settings2.Arrow.BarbWidth*(Factor)));
		this.BarbLength=Settings1.Arrow.BarbLength;
		this.ShaftWidth=Settings1.Arrow.ShaftWidth;
		this.HeadLength=Settings1.Arrow.HeadLength;
	}
...
function CMItemRect(TheType) 
{
	...
	case CMItemRect.ROUNDED_RECTANGLE:
		this.SetName("Rounded Rectangle");
		break;
	}
...
//		TheView.TheContext.strokeStyle = "black";
...
CMLayer.SettingDefintions=
{
	Layer:
	{
		...
		Metadata: { Name:"Metadata",Type:CMBase.DATA_TYPE_STRING, Default:"Undefined" },
		MinZoom: { Name:"Min Zoom",Type:CMBase.DATA_TYPE_FLOAT, Default:0 },
		MaxZoom: { Name:"Max Zoom",Type:CMBase.DATA_TYPE_FLOAT, Default:0 },
		ZoomToBoundsOnLoad: { Name:"Zoom To Bounds On Load",Type:CMBase.DATA_TYPE_BOOLEAN, Default:false },
	}, 
	Mark: 
	{ 
...
function CMLayer() 
{
	...
	//this.Clickable=true; // jjg - move to settings, false to make the layer so it does nothing when clicked on 
...
* @protected, @override
*//*
CMLayer.prototype.UpdateSettingsFromAttributes=function(FeatureIndex)
{
	if (this.SettingsDirty[FeatureIndex])
	{
...
* @protected
*/
CMLayer.prototype.GetSelectable=function() 
{
	return(this.GetSetting("Item","Selectable",true));
};
/**
...
CMLayer.prototype.OnLoad=function() 
{
	...
	else 
	{
		...
		if (this.GetSetting("Layer","ZoomToBoundsOnLoad")==true)
		{
			var TheMainContainer=this.GetParent(CMMainContainer);
...
CMLayer.prototype.MouseDown=function(TheView,RefX,RefY) 
{
	...
	if ((this.IsVisible())&&(this.GetSelectable())&&
		((TheView.GetTool()==CMView.TOOL_INFO)||(TheView.GetTool()==CMView.TOOL_SELECT))) // check if we where clicked in
	{
...
CMLayer.prototype.ProjectBounds=function(Bounds) 
{
	...
	{
		var TheProjector=TheGeo.GeProjector();
...
{
	var TheIcon=this.GetSetting("IconImage","TheImage");
...
CMLayer.prototype.FillPopupMenu=function(ThePopupMenu)
{
	...
	if (Metadata!=undefined)
	{
		...
		ZoomToElement.className="CM_LayerListPopupMenuItem";
		ZoomToElement.setAttribute("id","CM_ZoomToExtentMenuItem");
		ZoomToElement.innerHTML="Metadata";
...
//		this.SelectedFeatureIndex=-1;
		this.TheDataset.UnselectAll(SendMessageFlag);
...
/*
CMLayerDataset.prototype.CMLayer_SetSettingAttribute=CMLayer.prototype.SetSettingAttribute;

CMLayerDataset.prototype.SetSettingAttribute=function(Group,Key,AttributeName)
{
	this.CMLayer_SetSettingAttribute(Group,Key,AttributeName);
	
	// make sure the attribute is loaded into the dataset
...
CMLayerDataset.prototype.In=function(TheView,RefX,RefY) 
{
	...
	if (this.TheDataset!=null)
	{
		...
		if (Result!=null)
		{
			...
			{
				var Visible=this.GetFeatureSetting("Item","Visible",i,true);
...
CMLayerDataset.prototype.SetData=function(TheData,DataSetType) 
{
	...
	// create the new data set or get an existing one
	var TheScene=this.GetParent(CMScene);
	this.TheDataset=CMDataset.GetDataObject(null,DataSetType);
	this.TheDataset.SetParent(TheScene); 
...
*/	
	this.GetDataset(); // allocates the dataset
...
* @module CMLayerGraticuleGetProjectedCoordinateGrid 
******************************************************************************************************************/
...
// CMItem functions
//***********************************************************************
/*CMLayerGraticule.prototype.CMItem_SetVisible=CMItem.prototype.SetVisible;

CMLayerGraticule.prototype.SetVisible=function(Flag)
{
	if (this.GetVisible()!=Flag)
	{
		this.CMItem_SetVisible(Flag);
		
		if (typeof(CM3Scene)!== 'undefined')
		{
			var TheScene=this.GetParent(CM3Scene);
			
			if ((TheScene!=null)&&(this.Lines3D!=null))
			{
				var TheMesh=this.Lines3D;
				//var TheMesh=this.TheFeatureMeshes[i];
				
				if (Flag)  // add the mesh to the TheOGLScene to make it visible
				{
					TheScene.GetGeo().AddOGLObjectToGeo(TheMesh);
				}
				else // remove the meshfrom the TheOGLScene to make it invisible
				{
					TheScene.GetGeo().RemoveOGLObjectFromGeo(TheMesh);
				}
			}
		}
	}
...
//	var Projected2=TheProjector.ProjectFromGeographic(CenterLatitude,Lat2);
	...
	if ((Projected1!=null)&&(Projected2!=null))
	{
		...
		var GraticuleWidthInDegrees=DegreesPerPixel*this.GetSetting("Graticule","Spacing",200); // deisred width
...
CMLayerGraticule.prototype.GetLatLonRangeFromClippingBounds=function(TheView,LatLonRange,ClippingPoly,TheProjector) 
{
	var Result=null;
...
{
	// 2D painting
	if ((this.IsVisible())&&(TheView instanceof CMView2D))
	{
		// make sure we have a projector
		var TheGeo=this.GetParent(CMGeo);
		...
		if (ClippingPolys!=undefined)
		{
			...
			for (var i=0;i<ClippingPolys.length;i++)
			{
				...
				LatLonRanges.push(this.GetLatLonRangeFromClippingBounds(TheView,LatLonRange,ClippingPoly,TheProjector));
				
				//console.log(i+": "+LatLonRanges[i].MinLon+", "+LatLonRanges[i].MaxLon);
			}
			//**********************************************************************************************
...
CMLayerItems.prototype.DeleteAllChildren=function() 
{
	this.TheItems=[];
}
...
CMLayerItems.prototype.In=function(TheView,RefX,RefY) 
{
	...
	if ((this.IsVisible())&&(this.TheItems!=null))
	{
		...
		for (var i=0;( i <this.TheItems.length)&&(ItemIndex==-1); i++) 
		{
			...
			var Part=this.TheItems[i].InPart(TheView,RefX,RefY,RefTolerance);
...
CMLayerItems.prototype.MouseDown=function(TheView,RefX,RefY,TheEvent) 
{
	...
	if ((this.IsVisible())&&(this.GetSelectable())) // check if we where clicked in
	{
...
CMLayerRaster.SettingDefintions=
{
	Raster: 
	{ 
		Bounds: { Name:"URL",Type:CMBase.DATA_TYPE_BOUNDS, Default:null}, // Bounds object for the raster with XMin,XMax,YMin,YMax values
	},
};
...
CMLayerGrid.prototype.Paint=function(TheView) 
{
	...
	if ((this.IsVisible())&&(this.TheImageData!=null))
	{
		// Get the drawing context from our <canvas> and
		// set the fill to determine what color our map will be.
		var TheContext=TheView.GetContext();
...
CMMainContainer.SettingDefintions=
{
	MainContainer:
	{
		AllowMouseEvents: { Name:"Allow Mouse Events",Type:CMBase.DATA_TYPE_BOOLEAN, Default:true },
		MobileSupported: { Name:"Mobile Supported",Type:CMBase.DATA_TYPE_BOOLEAN, Default:false },
		Scene3D: { Name:"3D Scene",Type:CMBase.DATA_TYPE_BOOLEAN, Default:false },
		ImageFolder: { Name:"Image Folder",Type:CMBase.DATA_TYPE_STRING, Default:"../Images/" },
		CoordinateUnits: { Name:"Coordinate Units",Type:CMBase.DATA_TYPE_ENUMERATED,Options:[CMUtilities.COORDINATE_UNITS_DD,CMUtilities.COORDINATE_UNITS_DMS,CMUtilities.COORDINATE_UNITS_METERS,CMUtilities.COORDINATE_UNITS_FEET,CMUtilities.COORDINATE_UNITS_PIXELS,CMUtilities.COORDINATE_UNITS_ZOOM],Default:CMUtilities.COORDINATE_UNITS_DMS},
	},
};
//***************************************************************************************
// Constructors
...
{ 
	this.SetSetting("MainContainer","MobileSupported",MobileSupported);
...
CMMainContainer.prototype.Initialize=function(TheView)
{
	...
	if ((TheView==undefined)||(TheView instanceof CMView2D)) // no view specified or provided a 2D view
	{
		...
		if (TheView==undefined) TheView=TheView=new CMView2D(); // default
...
//		TheView.Setup(this.Elements[CMMainContainer.CANVAS]);
	...
	/*	var SettingsPanelElement=this.Elements[CMMainContainer.SETTINGS_PANEL];
		if  (CMUtilities.IsDefined(SettingsPanelElement)) 
		{
			...
			var TheSettingsTab=this.TabPanel.AddTab("Settings",ImageFolder+"Icon_Settings_Default.png",null,SettingsPanelElement,MapContainer);
	...
	*/
	...
	if  (CMUtilities.IsDefined(HoriziontalTabContainerElement)) 
	{
		...
		if  (CMUtilities.IsDefined(SettingsPanelElement)) 
		{
			var TheSettingsPanel=new CMPanelSettings(this);
			TheSettingsPanel.SetElement(SettingsPanelElement);
...
//***************************************************************************************
//	CMNorthArrow
//  JS class for creating north arrows that can point north.
//
//	By: Jim Graham 
//
//  Copyright 2018, Jim Graham
//
//  This file is part of CanvasMap.
//  
//  CanvasMap is free software: you can redistribute it and/or modify
//  it under the terms of the GNU General Public License as published by
//  the Free Software Foundation, either version 3 of the License, or
//  (at your option) any later version.
//  
//  CanvasMap is distributed in the hope that it will be useful,
//  but WITHOUT ANY WARRANTY; without even the implied warranty of
//  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
//  GNU General Public License for more details.
//  
//  You should have received a copy of the GNU General Public License
//  along with CanvasMap.  If not, see <https://www.gnu.org/licenses/>.
//
//***************************************************************************************
CMNorthArrow.SettingDefintions=
{
	NorthArrow:
	{
		...
		ClassName: { Name:"Class Name",Type:CMBase.DATA_TYPE_STRING, Default:"CM_NorthArrow" },
...
CMPanelBackgrounds.prototype.Setup=function(TheScene)
{
	...
	for (var i=0;(i<TheGeo.GetNumBackgrounds());i++)
	{
		...
		//if (i!=TheGeo.SelectedBackgroundIndex) TheBackground.SetVisible(false);
...
/*				var Text=TheView.GetCoordinateString(RefX,RefY,ThePanelFooter.CoordinateUnits);
				...
				var TheProjector=ThePanelFooter.TheMainContainer.GetProjector();
...
/*			var Xs=[];
			...
			Xs.push(ClippingBox.Xs[0]); // upper left
			Ys.push(ClippingBox.Ys[0]);
			
			Xs.push(ClippingBox.Xs[1]); // upper right
			Ys.push(ClippingBox.Ys[0]);
			
			Xs.push(ClippingBox.Xs[1]); // lower right
			Ys.push(ClippingBox.Ys[1]);
			
			Xs.push(ClippingBox.Xs[0]); // lower left
			Ys.push(ClippingBox.Ys[1]);
*/			
...
//CanvasMap/js/CMProjectorProj4JS2.js
//***************************************************************************************
//	CMProjectorProj4JS2
//
// Projection class to use the Proj4 library that I converted from Proj4JS.  This version
// exposes traditional JavaScript functions.
// 
// Note that the library does not support reverse transforms in a number of cases so these
// projection methods are commented out.
//***************************************************************************************
...
* @public, @settings
*/
CMProjectorProj4JS2.SettingDefintions=
{
	Projection:
	{
		lon_0: { Name:"Longitude Of Origin",Type:CMBase.DATA_TYPE_FLOAT, Default:0, Min:-180,Max:180 },
		lon_1: { Name:"First Meridian",Type:CMBase.DATA_TYPE_FLOAT, Default:-40, Min:-180,Max:180 },
		lon_2: { Name:"Second Merdian",Type:CMBase.DATA_TYPE_FLOAT, Default:40, Min:-180,Max:180 },
		
		lat_ts: { Name:"Latitude Of True Scale",Type:CMBase.DATA_TYPE_FLOAT, Default:0, Min:-90,Max:90 },
		lat_0: { Name:"Latitude Of Origin",Type:CMBase.DATA_TYPE_FLOAT, Default:0, Min:-90,Max:90 },
		lat_1: { Name:"First Parallel",Type:CMBase.DATA_TYPE_FLOAT, Default:20, Min:-90,Max:90 },
		lat_2: { Name:"Second Parallel",Type:CMBase.DATA_TYPE_FLOAT, Default:40, Min:-90,Max:90 },
		
		Method: { Name:"Method",Type:CMBase.DATA_TYPE_ENUMERATED, 
			Options:[
				 'Albers Equal Area',
//				 'Azimuthal Equidistant Projection',
		//		 'Cassini',
				 'Cylindrical Equal-Area',
				 "Cylindrical Equal-Area Projection", // could not find in proj4 docs
				 'Equidistant Cylindrical (Plate Carree)',
				 'Equidistant conic projection',
	//			 "Gauss-Schreiber Transverse Mercator",
				 "Gnomonic",
	//			 'Lambert equal area', // only works with positive First Parallel values otherwise, negative latitudes give an error
...
//				 'Transverse Mercator',
	//			 'Van der Grinten Projection', // cannot project 30,0 and other simple coordinates
				], 
			Default:'Albers Equal Area' }, // only those projection methods that work well are exposed at this time.
	},
};
// These are the Proj4 names for the full projection methods that are listed above.
CMProjectorProj4JS2.MethodIDs=[
							   "aea",
//							   "aeqd",
		//					   "cass",
							   "cea",
							   "equi", //Cylindrical equal-area projection
							   "eqc",
							   "eqdc", //'Equidistant Cylindrical (Plate Carree)',
...
function CMProjectorProj4JS2() 
{
	...
	// used to find bounds of projection
	this.LatitudeOfOrigin=0;
	this.LongitudeOfOrigin=0;
	
	// additional settings are fixed for now (should be moved to settings)
...
CMProjectorProj4JS2.prototype=Object.create(CMProjector.prototype); // inherit prototype functions from CMProjector()

CMProjectorProj4JS2.prototype.contructor=CMProjectorProj4JS2; // override the constructor to go to ours
//***************************************************************************************
//	Private functions
//***************************************************************************************
/**
* @private
*/
CMProjectorProj4JS2.prototype.SetupProjectionName=function(NewName)
{
	this.ProjectionName=CMProjectorProj4JS2.GetProjectionCodeFromProjectionName(NewName);
}
/**
* @private
*/
CMProjectorProj4JS2.prototype.SetupRanges=function()
{
	this.XRange=180;
	this.YMax=90;
	this.YMin=-90;
	...
	case "aea": // Albers equal area
		//this.Projection=aea; // lon_0, lat1, lat2
		this.XRange=120;
		this.YMax=this.GetSetting("Projection","lat_1")+40;
		this.YMin=this.GetSetting("Projection","lat_2")-40;
		break;
	case "aeqd": // azimuthal equidistant projection
		//this.Projection=aeqd; // lat_ts, lat_0, lon_0
		this.XRange=80;
		TheLatitude=this.GetSetting("Projection","lat_0");
		this.YMax=TheLatitude+50;
		this.YMin=TheLatitude-50;
		break;
	case "cass": // Cassini
		//this.Projection=cass; // Cassini, lon_0, lat_0
		this.XRange=70;
		TheLatitude=this.GetSetting("Projection","lat_0");
		this.YMax=TheLatitude+80;
		this.YMin=TheLatitude-80;
		break;
	case "cea": //  cylindrical equal-area
		//this.Projection=cea; // lat_ts, lon_0
		break;
	case "eqc": // Equidistant Cylindrical (Plate Carree) lon_0 lat_0 lat_ts
		//this.Projection=eqc; // 
		TheLatitude=this.GetSetting("Projection","lat_0");
		this.YMax=TheLatitude+80;
		this.YMin=TheLatitude-80;
		break;
	case "eqdc": // Equidistant conic projection lat_1 lat_2 lon_0 
		...
		this.YMin=TheLatitude-80;
		break;
	case "equi": // Cylindrical equal-area projection lat_ts lon_0
		//this.Projection=equi; // 
		break;
	case "gauss": // Gauss-Kruger Projection
		//this.Projection=gauss; //  // does not work
		break;
	case "gnom": // Gnomonic lon_0
		TheLatitude=this.GetSetting("Projection","lat_0"); // should be near -90
		this.XRange=180;
		TheLatitude=this.GetSetting("Projection","lat_0");
		if (TheLatitude>=0)
		{
		...
		break;
	case "gstmerc": // Gauss-Schreiber Transverse Mercator (aka Gauss-Laborde Reunion) lon_0 lat_0
		...
		break;
	case "krovak": // krovak
		//this.Projection=krovak; //  lon_0 lat_0
		this.XRange=70;
		TheLatitude=this.GetSetting("Projection","lat_0");
		this.YMax=80;
		this.YMin=0;
		break;
	case "laea": // Lamber equal area
		//this.Projection=laea; // lat_1 south (se indiates second paralelle is 90S instead of 90N) lon_0
		this.XRange=120;
		TheLatitude=this.GetSetting("Projection","lat_1");
		this.YMax=TheLatitude+90;
		this.YMin=TheLatitude-90;
		break;
	case "lcc": // Lambert conformal conic  lon_0 lat_0 lat_1 lat_2
		//this.Projection=lcc; // 
		this.XRange=120;
		this.YMax=this.GetSetting("Projection","lat_1")+40;
		this.YMin=this.GetSetting("Projection","lat_2")-40;
		break;
	case "longlat": // 
		//this.Projection=longlat; //  // does not work
		break;
	case "merc": // mercator lat_ts lon_0
		//this.Projection=merc; // 
		this.XRange=180;
		this.YMax=85;
		this.YMin=-85;
		break;
	case "mill": // Miller Cylindrical Projection lon_0
		//this.Projection=mill; // 
		break;
	case "moll": // Mollweide projection lon_0
		//this.Projection=moll; // 
		break;
	case "nzmg": // New Zealand map grid
		//this.Projection=nzmg; //  
		break;
	case "omerc": // Oblique Mercator lon_1 lat_1
	...
	case "poly": // polyconic
		...
		this.XRange=120;
		break;
	case "sinu": // Sinusoidal projection
		//this.Projection=sinu; // 
		break;
	case "somerc": // Swiss Oblique Mercator(looks cylindrical)
		//this.Projection=somerc; // 
		this.XRange=80;
		this.YMax=80;
		this.YMin=-80;
		break;
	case "stere": // Stereographic
		//this.Projection=stere; // 
		this.XRange=90;
		break;
	case "sterea": // Stereographic projection
		//this.Projection=sterea; // 
		this.XRange=100;
		this.YMax=80;
		this.YMin=-80;
		break;
	case "tmerc": // Transverse Mercator
	...
	case "utm": // just runs tmerc
		...
		break;
	case "vandg": // van der Grinten Projection
		this.XRange=120;
		this.YMax=80;
		this.YMin=-80;
		break;
/*		case "robin": //robinson?
		this.Projection=robin; // 
		break;
*/	}
...
CMProjectorProj4JS2.prototype.GetSettingsDefinitions=function() 
{
	var Result={};
	
	for (Key in CMProjectorProj4JS2.SettingDefintions)
	{
		Result[Key]=CMProjectorProj4JS2.SettingDefintions[Key];
	}

	return(Result); 
}
CMProjectorProj4JS2.prototype.CMProjector_SettingsChanged=CMProjector.prototype.SettingsChanged;

CMProjectorProj4JS2.prototype.SettingsChanged=function() 
{
	this.CMProjector_SettingsChanged();
	
	this.SendMessageToListeners(CMBase.MESSAGE_SETTINGS_CHANGED);
...
CMProjectorProj4JS2.prototype.Initialize=function()
{
	...
	{
		var ProjectionName=this.GetSetting("Projection","Method");
		
		// used to compute bounds
		this.LongitudeOfOrigin=this.GetSetting("Projection","lon_0");
		this.LatitudeOfOrigin=this.GetSetting("Projection","lat_0");
		
		this.SetupProjectionName(ProjectionName);
		...
		case "aeqd": // azimuthal equidistant projection
			...
			break;
		case "cass": // Cassini
			this.Projection=cass; // Cassini
			break;
		case "cea": //  cylindrical equal-area
			this.Projection=cea; // k0
			break;
		case "eqc": // Equidistant Cylindrical (Plate Carree)
			this.Projection=eqc; // 
			break;
		case "eqdc": // Equidistant conic projection
			this.Projection=eqdc; // 
			break;
		case "equi": // Cylindrical equal-area projection
			this.Projection=equi; // 
			break;
		case "gauss": // Gauss-Kruger Projection
			this.Projection=gauss; //  // does not work
		...
		case "gnom": // Gnomonic
			...
			break;
		case "gstmerc": // Gauss-Schreiber Transverse Mercator (aka Gauss-Laborde Reunion)
			this.Projection=gstmerc; // 
			break;
		case "krovak": // krovak
			this.Projection=krovak; // 
			break;
		case "laea": // Lambert equal area
			this.Projection=laea; // 
			break;
		case "lcc": // Lambert conformal conic 
			this.Projection=lcc; // 
			break;
		case "longlat": // 
			this.Projection=longlat; //  // does not work
		...
		case "merc": // mercator
			this.Projection=merc; // 
			break;
		case "mill": // Miller Cylindrical Projection
			this.Projection=mill; // 
			break;
		case "moll": // Mollweide projection
			this.Projection=moll; // 
			break;
		case "nzmg": // New Zealand map grid
			this.Projection=nzmg; //  
			break;
		case "omerc": // Oblique Mercator
			this.Projection=omerc; // 
			break;
		case "ortho": // orthographic
			this.Projection=ortho; // 
...
*/		}
		...
		this.Projection.lat_ts=CMProjector.DegreesToRadians(this.GetSetting("Projection","lat_ts")); // 
		this.Projection.lat0=CMProjector.DegreesToRadians(this.GetSetting("Projection","lat_0")); // latitude of origin
		this.Projection.lat1=CMProjector.DegreesToRadians(this.GetSetting("Projection","lat_1"));
		this.Projection.lat2=CMProjector.DegreesToRadians(this.GetSetting("Projection","lat_2"));
		
		if (this.Projection==gnom) // lat of origin can only be 90 or -90 for Gnomic
		{
			var Temp=this.GetSetting("Projection","lat_0");
			
			if (Temp>=0) this.Projection.lat0=Math.PI/2;
			else this.Projection.lat0=-Math.PI/2;
		}
		this.Projection.long0=CMProjector.DegreesToRadians(this.GetSetting("Projection","lon_0")); // longitude of origin
		this.Projection.long1=CMProjector.DegreesToRadians(this.GetSetting("Projection","lon_1"));
		this.Projection.long2=CMProjector.DegreesToRadians(this.GetSetting("Projection","lon_2"));

/*		this.Projection.lat_ts=0; // 
		this.Projection.lat0=0; // latitude of origin
		this.Projection.lat1=0; // 20 for Albers Equal area
		this.Projection.lat2=0; // 40 for Albers equal area
		
		this.Projection.long0=0; // longitude of origin
//		this.Projection.long1=-40;
//		this.Projection.long2=40;
*/	
...
*
*/
CMProjectorProj4JS2.prototype.ProjectFromGeographic=function(Lon,Lat,Elevation)
{
	if (Lon==undefined) throw("something went wrong with the coordinates in CMProjectorProj4JS2.ProjectFromGeographic()");
																													   
	this.Initialize();
	
	if (Lat>=90) Lat=89.99999;
	if (Lat<=-90) Lat=-89.99999;
	if (Lon<=-180) Lon=-179.99999;
	if (Lon>=180) Lon=179.99999;
	
	if ((Lat>90)||(Lat<-90)||(Lon<-180)||(Lon>180))
	{
		var j=12; // jjg - this keeps the compiler from throwing a warning
//		throw("Coordinate out of bounds in");
	}
...
/*	if (isNaN(Coordinate.x)||isNaN(Coordinate.x)) 
	{
		throw("Sorry, the geographic coordinate "+Lat+", "+Lon+" resulted in a not number value");
	}
*/	var Result=null;
	
	if ((Coordinate!=null)&&((isNaN(Coordinate.x)==false)&&(isNaN(Coordinate.x)==false)))
	{
	  Result=[Coordinate.x,Coordinate.y,Elevation];
	}
	
	return(Result);
};

CMProjectorProj4JS2.prototype.ProjectToGeographic=function(X,Y,Elevation)
{
	var Result=null;
...
* @returns - 
*/
CMProjectorProj4JS2.prototype.GetClippingPolys=function()
{
	var Result=null;
	
	this.Initialize();

	var XMin=this.LongitudeOfOrigin-this.XRange;
	var XMax=this.LongitudeOfOrigin+this.XRange;
	
	if (this.LongitudeOfOrigin!=0)  // have to clip
	{
		var Pad=0.00001;
		
		Result=CMProjector.GetGeographicClippingPolys(XMin,XMax,this.YMin,this.YMax,Pad);
	}
...
* @returns A Region that can be drawn with TheView.PaintRefRegion(RegionToDraw,false,false,true)
*/
CMProjectorProj4JS2.prototype.CMProjector_GetRegionToDraw=CMProjector.prototype.GetRegionToDraw;

CMProjectorProj4JS2.prototype.GetRegionToDraw=function()
{
	var RegionToDraw=[];
	
	if (this.ProjectionName=="ortho")
	{
		var PolyToDraw=CMUtilities.GetRegularPolygon(100,this.SemiMajor*2,0,0,0);
		var AreaToDraw=[PolyToDraw];
		RegionToDraw.push(AreaToDraw);
	}
	else
	{
		RegionToDraw=this.CMProjector_GetRegionToDraw();
	}
	return(RegionToDraw);
}

/**
* Return a region that can be used to fill the area of the Geo based on the current projection
* @public
* @returns A Region that can be filled with TheView.PaintRefRegion(RegionToFill,false,true,false);
*/
CMProjectorProj4JS2.prototype.CMProjector_GetRegionToFill=CMProjector.prototype.GetRegionToFill;

CMProjectorProj4JS2.prototype.GetRegionToFill=function()
{
	var RegionToFill=[];
	...
	if (this.ProjectionName=="ortho")
	{
		var PolyToDraw=CMUtilities.GetRegularPolygon(100,this.SemiMajor*2,0,0,0);
		var AreaToDraw=[PolyToDraw];
		RegionToFill.push(AreaToDraw);
	}
	else
	{
		RegionToFill=this.CMProjector_GetRegionToFill();
	}
	return(RegionToFill);
}
/**
* Gets the maximum rectangular box for the current projection
* Used to zoom to the entire bounds of a projection
*/
CMProjectorProj4JS2.prototype.CMProjector_GetProjectedBounds=CMProjector.prototype.GetProjectedBounds;

CMProjectorProj4JS2.prototype.GetProjectedBounds=function()
{
	var Result=null;
	...
	{
		Result=this.CMProjector_GetProjectedBounds();
	}
	return(Result);
}

//***************************************************************************************
// public CMProjectorProj4JS2 Functions
//***************************************************************************************

/*
* Should be called internally when the projection name changes
*/
CMProjectorProj4JS2.prototype.ResetSettings=function()
{
	this.SetSetting("Projection","lon_0",0);
	this.SetSetting("Projection","lon_1",0);
	this.SetSetting("Projection","lon_2",0);
	this.SetSetting("Projection","lat_0",0);
	this.SetSetting("Projection","lat_1",40);
	this.SetSetting("Projection","lat_2",-40);
	this.SetSetting("Projection","lat_ts",0);
}
CMProjectorProj4JS2.prototype.GetClass=function() { return(CMProjectorProj4JS2); }

//***************************************************************************************
// static public CMProjectorProj4JS2 Functions
//***************************************************************************************
/**
* @static, @private
*/
CMProjectorProj4JS2.GetProjectionCodeFromProjectionName=function(NewName)
{
	var SettingsNames=CMProjectorProj4JS2.SettingDefintions.Projection.Method.Options;
	
	var Index=SettingsNames.indexOf(NewName);
	
	return(CMProjectorProj4JS2.MethodIDs[Index]);
}
/**
* @public, @static
*/
CMProjectorProj4JS2.ControlDefinitions=
{
	lon_0: {
		 Name:"lon_0",
		 Values:[-180,180]
...
CMProjectorProj4JS2.GetControlInformation=function(ProjectionCode)
{
	var Controls=null;
	var Defs=CMProjectorProj4JS2.ControlDefinitions;
	
	switch (ProjectionCode)
	{
	case "aea": // Albers equal area
		Controls=[Defs.lon_0,Defs.lat_1,Defs.lat_2]; //Defs["lon_0"],Defs.lat_1,Defs.lat_2
		break;
	case "aeqd": // azimuthal equidistant projection
		Controls=[Defs.lon_0,Defs.lat_0]; // "lat_ts", has not effect
		break;
	case "cass": // Cassini
		Controls=[Defs.lon_0,Defs.lat_0];
		break;
	case "cea": //  cylindrical equal-area
		Controls=[Defs.lon_0,Defs.lat_ts];
		break;
	case "eqc": // Equidistant Cylindrical (Plate Carree) lon_0 lat_0 lat_ts
		Controls=[Defs.lon_0,Defs.lat_0,Defs.lat_ts];
		break;
	case "eqdc": // Equidistant conic projection lat_1 lat_2 lon_0 
		Controls=[Defs.lon_0,Defs.lat_1,Defs.lat_2];
		break;
	case "equi": // Cylindrical equal-area projection lat_ts lon_0
		Controls=[Defs.lon_0]; //,"lat_ts"
		break;
	case "gauss": // Gauss-Kruger Projection
		//this.Projection=gauss; //  // does not work
		break;
	...
	case "laea": // Lamber equal area
		//this.Projection=laea; // lat_1 south (se indiates second paralelle is 90S instead of 90N) lon_0
		Controls=[Defs.lon_0,Defs.lat_1];
		break;
	case "lcc": // Lambert conformal conic  lon_0 lat_0 lat_1 lat_2
		Controls=[Defs.lon_0,Defs.lat_1,Defs.lat_2]; // "lat_0", just moves the result up and down
		break;
	case "longlat": // 
		//this.Projection=longlat; //  // does not work
		break;
	...
	case "nzmg": // New Zealand map grid
		...
		break;
	case "omerc": // Oblique Mercator lon_1 lat_1
		Controls=[Defs.lon_1,Defs.lon_2,Defs.lat_1,Defs.lat_2];
		break;
	case "ortho": // orthographic
		Controls=[Defs.lon_0,Defs.lat_0];
		break;
	...
	case "sinu": // Sinusoidal projection
		Controls=[Defs.lon_0];
		break;
	case "somerc": // Swiss Oblique Mercator
		Controls=[Defs.lon_0];
		break;
	case "stere": // Stereographic
		Controls=[Defs.lon_0,Defs.lat_ts];
		break;
	case "sterea": // Oblique Stereographic Alternative
		Controls=[Defs.lon_0,Defs.lat_0];
		break;
	case "tmerc": // Transverse Mercator
		Controls=[Defs.lon_0,Defs.lat_0];
...
function CMGeo(TheScene) 
{
	...
	this.SetName("Geo"+CMGeo.NumGeos);
	CMGeo.NumGeos++;
	
	this.SetParent(TheScene);
	
	// contained objects
 	this.Layers=[]; // this is the Geo's children
	
	this.Backgrounds=[]; // move to Geos?
	this.SelectedBackgroundIndex=-1;
	
	this.TheProjector=null;
	this.ProjectorType=CMGeo.PROJECTOR_NONE;
	
	this.MinTime=0; 
	
	this.SetSetting("Style","fillStyle","rgb(210,220,255)"); // only works if there is a projection
//	this.SetSettingGroup("Style",undefined); // turn off drawing the Geo by default
...
* @returns LayerIndex
*/
CMGeo.prototype.GetLayerIndex=function(TheLayer) 
{
	var Result=-1;
...
*/
CMGeo.prototype.DeleteLayer=function(Index) 
{
...
*/
CMGeo.prototype.MouseDown=function(TheView,RefX,RefY,TheEvent) 
{
	var Used=false;
	
	var NumLayers=this.GetNumChildren(CMLayer);
	
	for (var i=NumLayers-1;(i>=0)&&(Used==false);i--)
	{
		var TheLayer=this.GetChild(i);
		
		Used=TheLayer.MouseDown(TheView,RefX,RefY,TheEvent);
	}
	return(Used);
};
/**
...
* @param RefY - reference vertical coordinate value
*/
CMGeo.prototype.MouseMove=function(TheView,RefX,RefY,TheEvent) 
{
	var Used=false;
	...
	for (var i=NumLayers-1;(i>=0)&&(Used==false);i--)
	{
		...
		Used=TheLayer.MouseMove(TheView,RefX,RefY,TheEvent);
	}
	return(Used);
};
/**
...
* @param RefY - reference vertical coordinate value
*/
CMGeo.prototype.MouseUp=function(TheView,RefX,RefY,TheEvent) 
{
	var Used=false;
	...
	for (var i=NumLayers-1;(i>=0)&&(Used==false);i--)
	{
		...
		Used=TheLayer.MouseUp(TheView,RefX,RefY,TheEvent);
	}
	return(Used);
};
...
CMGeo.prototype.UnselectAll=function(SendMessageFlag) 
{
	...
	for (var i=0;i<NumLayers;i++)
	{
		...
		TheLayer.UnselectAll(SendMessageFlag);
	}
}
...
/*	this.TimeSlices[0].Settings.Fill=	
	{
...
//	this.SetSetting("Style","fillStyle","rgb(210,220,255)");
	...
	this.SetName("Scene_"+CMScene.NumScenes);
}
...
CMScene.prototype.GetNumChildren=function(TheClass)  
{ 
	var Result=0;
	
	if (this.Geos!=null) Result=this.Geos.length;
	
	return(Result);
}

CMScene.prototype.GetChild=function(Index,TheClass)  
{ 
	return(this.Geos[Index]);
}
...
{
	var Index=this.Geos[0].AddBackground(TheLayer);
...
{
	var Index=this.Geos[0].SetSelectedBackgroundIndex(New);
}
...
CMScene.prototype.AddLayer=function(TheLayer,GeoIndex) 
{
	...
	var LayerIndex=this.Geos[GeoIndex].AddLayer(TheLayer);
...
* @returns LayerIndex
*/
CMScene.prototype.GetLayerIndex=function(TheLayer,GeoIndex) 
{
	if (GeoIndex==undefined) GeoIndex=0;
	
	var Result=this.Geos[GeoIndex].GetLayerIndex(TheLayer);
	return(Result);
}
/*
* Returns the layer at the specified index.
...
CMScene.prototype.GetLayer=function(LayerIndex,GeoIndex) 
{
	...
	var Result=this.Geos[GeoIndex].GetLayer(LayerIndex);
...
CMScene.prototype.MoveLayerUp=function(Index,GeoIndex) 
{
	...
	var Result=this.Geos[GeoIndex].MoveLayerUp(Index);
}
/*
* Swap the specified layer with the one below it.
* @public
...
CMScene.prototype.MoveLayerDown=function(Index,GeoIndex) 
{
	...
	var Result=this.Geos[GeoIndex].MoveLayerDown(Index);
}
...
CMScene.prototype.DeleteLayer=function(Index,GeoIndex) 
{
	...
	var Result=this.Geos[GeoIndex].DeleteLayer(Index);
	return(Result);
}
/**
* Moves a layer to a new position in the layer list 
...
CMScene.prototype.MoveLayer=function(TheLayer,NewIndex,GeoIndex) 
{
	...
	this.Geos[GeoIndex].MoveLayer(TheLayer,NewIndex);
}
//******************************************************************
// Search
//******************************************************************
...
{
	this.Geos[0].GetSearchResults(TheText,SearchResults);
...
CMScene.prototype.In=function(TheView,RefX,RefY,TheEvent) 
{
	...
	for (var i=NumGeos-1;(i>=0)&&(Result==null);i--)
	{
		...
		var Result=TheLayer.In(TheView,RefX,RefY,TheEvent);
...
CMScene.prototype.MouseDown=function(TheView,RefX,RefY,TheEvent) 
{
	...
	for (var i=NumGeos-1;(i>=0)&&(Used==false);i--)
	{
		var TheGeo=this.GetChild(i);
		
		Used=TheGeo.MouseDown(TheView,RefX,RefY,TheEvent);
	}
	return(Used);
};
/**
...
CMScene.prototype.MouseMove=function(TheView,RefX,RefY,TheEvent) 
{
	...
	for (var i=NumGeos-1;(i>=0)&&(Used==false);i--)
	{
		...
		Used=TheGeo.MouseMove(TheView,RefX,RefY,TheEvent);
	}
	return(Used);
};
/**
...
CMScene.prototype.MouseUp=function(TheView,RefX,RefY,TheEvent) 
{
	...
	for (var i=NumGeos-1;(i>=0)&&(Used==false);i--)
	{
		...
		Used=TheGeo.MouseUp(TheView,RefX,RefY,TheEvent);
	}
	return(Used);
};
...
CMScene.prototype.Paint=function(TheView) 
{
	...
	{
		this.GetParent(CMMainContainer).AddToDebugPanel("CMScene.Paint Starting:"+CMUtilities.GetSeconds());
		...
		this.GetParent(CMMainContainer).AddToDebugPanel("CMScene.Paint Done: "+CMUtilities.GetSeconds());
		
		if (this.NeedRepaint) this.Repaint();
	}
	else
	{
		this.GetParent(CMMainContainer).AddToDebugPanel("CMScene.Paint NeedRepaint=true: "+CMUtilities.GetSeconds());
...
//	this.GetParent(CMMainContainer).AddToDebugPanel("CMScene.Repaint Exit:"+CMUtilities.GetSeconds());
}
/**
...
* @param TheProjector - the projector to use to project coordinates
*/
CMScene.prototype.SetProjector=function(TheProjector) 
{ 
	
	if (this.Geos!=null) this.Geos[0].SetProjector(TheProjector);
	else alert("Sorry, the Geo must be setup before the projector can be set");
}
CMScene.prototype.GetProjector=function() 
{ 
	var TheProjector=null;
	
	if (this.Geos!=null) TheProjector=this.Geos[0].GetProjector();
...
* @private
*/
CMTile.prototype.GetTileImageFilePath=function(Extension) 
{
...
{
//	var FileName=this.GetTileImageFileName();
...
//	var ThePath=this.TheDataset.URL+FileName;
	...
	var ThePath=this.GetTileImageFilePath(".png");
	...
	this.TheRasterRequest=
	{
		...
		TheFunction:function()
		{ 
			//this.TheTile.SendLoadMessageToListeners(); // for texture tiles
				
			this.TheTile.TheDataset.SendMessageToListeners(CMDataset.MESSAGE_DATASET_TILE_LOADED,this.TheTile);
			//this.OnLoadFunction(this.TheTile,this.TheView);
//			this.TheTile.TheDataset.GetParent(CMScene).Repaint();  // jjg - this triggers the repaint
		}
	};
	this.TheRasterRequest.TheTile=this;
...
* @private
*/
CMTile.GetDistanceToView=function(TheView,Extent) 
{
	var CameraPosition=TheView.GetPosition();
	//var Extent=this.GetTileExtent();
...
CMTile.prototype.CheckPaintTile=function(TheView,ThisStepTileRefWidth) 
{
	...
	else // otherwise, check if this tile needs to be painted
	{
		if (PaintTile==false) // see if we should 
		{
			...
			if ((typeof(CM3View)!="undefined")&&(TheView instanceof CM3View))
			{
				var Distance=CMTile.GetDistanceToView(TheView,this.GetTileExtent());
...
* @private
*/
CMTile.prototype.GetTileExtent=function() 
{
	var Factor=1/Math.pow(2,this.ZoomLevel); // width of one pixel
	var TileRefWidth=256*Factor;
...
* @private
*/
CMTile.prototype.GetDataForTextureTile=function(TheView,TheTextureTile,Exaggeration)
{
	var Result=null;
	...
	{
		var RequestedExtent=TheTextureTile.GetTileExtent();
		var ThisExtent=this.GetTileExtent();
		...
		{			
			for (var Row=0;(Row<2)&&(Result==false);Row++)
			{
				for (Column=0;(Column<2)&&(Result==false);Column++)
				{
					...
					Result=TheChildTile.GetDataForTextureTile(TheView,TheTextureTile,Exaggeration);
				}
			}
		}
	}
...
/**
* Utility function to setup materials for the tiles.
* @private
*/
CMTile.prototype.SetupMaterial=function(TheImage)
{
	var texloader = new THREE.TextureLoader();

	// create the geometry and set the array into it to create the positions (verticies) and normals
	...
	SolidColorMaterial.map=texloader.load(TheImage); // image
...
//			alert("status="+this.status);
			if( this.status == 200) // OK
			{
				...
				{
					TheTile.LoadTileRaster(); // try to immeidately load the raster
				}
...
//	var Factor=1/Math.pow(2,this.ZoomLevel); // width of one pixel
	...
	if ((typeof(CM3View)!="undefined")&&(TheView instanceof CM3View)) // have a 3d view
	{
		...
		{
			PaintTile=this.CheckPaintTile(TheView,ThisStepTileRefWidth);
					...
					if (this.MeshIsInGeo==false) // the mesh is not in the Geo, we need to add it
					{
						if (this.TheMesh==null) // the mesh is not available, we need to create it
						{
							...
							if (TheMatrixes!=null)
							{
								...
								var TheGeometry=new CM3SurfaceGeometry(NumColumns-1,NumRows-1,TheMask); //TheMask);
								
								TheGeometry.SetPositions(TheArray);
								
								// setup the material
								var TheFileName=this.GetTileImageFilePath(".png");
								
								var TheMaterial=this.SetupMaterial(TheFileName);
							
								// setup the mesh
								this.TheMesh=new THREE.Mesh(TheGeometry,TheMaterial);
...
* @result - an object with an array of LegendLabels appropriate for a legend and an array of FeatureColors to colorize each of the features
*/
...
* @returns - average value of the values in the array
*/
...
* @public
* @param TheArray - array of numeric values
* @returns - an object with Min and Max entries
*/
...
* @returns - a histogram in an object with the keys:
...
* @public
* @param TheArray 
* @returns - Array with entries reversed.
*/
...
* @public
* @private Value - the DD value
* @private EastWestFlag - true if the DMS value is east/west, false for north/south
* @private NotHTML - true if the value will be displayed as HTML
* @private VariablePrecision - true to padd the DMS with zeros.
...
* @returns true if the interior bounds is inside the exterior bounds (they can touch and this can still be true)
*/
...
* @param TheBounds
* @param NewBounds
* @returns Bounds with the NewBounds included
*/
...
* @param Ys
* @param NumPoints
* @returns - Bounds surrounding the polygon
*/
...
* @returns numeric distance between the two points
*/
	...
	case CMUtilities.COORDINATE_UNITS_DD:
		{
			if (TheProjector!=null) //Text="Sorry, we need a projector to show geographic coordinates with projected data";
			{
				...
				{
					Text="Undefined Back Projection";
				}
			}
...
CMUtilityBezier.GetFourPoint3D=function(NumSteps,X0,Y0,Z0,X1,Y1,Z1,X2,Y2,Z2,X3,Y3,Z3)
{
	...
	var  Result=CMUtilityBezier.GetTwoSlope2D(NumSteps,X1,Y1,Z1,X2,Y2,Z2,DX1,DY1,DZ1,DX2,DY2,DZ2);
...
/*		
		...
		if (TheSticky!=null)
		{
			var TheCanvasElement=this.GetParent(CMMainContainer).GetElement(CMMainContainer.CANVAS);
...
*/
CMView.SetStyleToContext=function(NewStyle,SaveCurrentStyle,TheContext) 
{
	...
	if ((NewStyle!=null)&&(NewStyle!=undefined))
	{
		...
		for (var key in NewStyle)
		{
			...
			else if (key=="GradientType")
			{
				...
				var GradientRadius=NewStyle["GradientRadius"];
				
				if (GradientRadius==undefined) GradientRadius=100;
				...
				{
					TheGradient=TheContext.createRadialGradient(X0,Y0,X1,Y1,GradientRadius);
				}
...
{
	CMView.SetStyleToContext(NewStyle,SaveCurrentStyle,this.TheContext);
...
/*	// this has to be saved each time so the restores are synchronized
	...
	if ((NewStyle!=null)&&(NewStyle!=undefined))
	{
		...
		for (var key in NewStyle)
		{
			if (key=="PatternImage")
			{
				var ThePattern = this.TheContext.createPattern(NewStyle[key], "repeat");
				this.TheContext["fillStyle"]=ThePattern;
			}
			else if (key=="GradientType")
			{
				var TheColors=NewStyle["GradientColors"];
				...
				if (NewStyle[key]=="Linear")
				{
					TheGradient=this.TheContext.createLinearGradient(X0,Y0,X1,Y1);
				}
				else
				{
					TheGradient=this.TheContext.createRadialGradient(X0,Y0,X1,Y1,GradientRadius);
				}
				...
				this.TheContext["fillStyle"]=TheGradient;
			}
			else  if ((key=="GradientColors")||(key=="GradientCoordinates")||(key=="GradientRadius")) // ignore these as they are handled above
			{
			}
...
//	var TabContent=null;
	...
	if  (this.TheElement!=null)
	{
		...
		if (this.Tabs.length>1) // make all tabs but the first one hidden
		{
